...
from contracts.lib.storage_verification.xor_state import state_xor, mask_garbage
from contracts.lib.storage_verification.swap_endianness import swap_endianness_64
...
func keccak_f{range_check_ptr, keccak_ptr: felt*, bitwise_ptr: BitwiseBuiltin*}(input: felt*) -> (
    output: felt*
) {
...
func load_full_block{
    bitwise_ptr: BitwiseBuiltin*, range_check_ptr, keccak_ptr_start: felt*, keccak_ptr: felt*
...
func recursive_keccak{range_check_ptr, keccak_ptr: felt*, bitwise_ptr: BitwiseBuiltin*}(
    state: felt*, input: felt*, n_bytes: felt
) -> (output: felt*) {
    alloc_locals;
    ...
    if (n_bytes_in_current_block == 136) {
        ...
        let (state_update: felt*) = recursive_keccak(
            state=keccak_f_ptr, input=input + 17, n_bytes=n_bytes - n_bytes_in_current_block
        );
        return (state_update,);
        ...
        let (formatted_input: felt*) = load_block_with_padding{keccak_ptr_start=keccak_ptr}(
            input=input, n_bytes=n_bytes_in_current_block, n_word=17
        );
        let (xor: felt*) = state_xor(state, formatted_input);
        let (keccak_f_ptr: felt*) = keccak_f(input=xor);
        return (keccak_f_ptr,);
