...
from contracts.lib.secp.bigint import BASE, BigInt3, UnreducedBigInt3, bigint_mul, nondet_bigint3
from contracts.lib.secp.secp_def import N0, N1, N2
from contracts.lib.secp.secp_ec import EcPoint, ec_add, ec_mul
from contracts.lib.secp.secp_field import unreduced_mul, unreduced_sqr, verify_zero
...
func ecdsa_raw_recover{range_check_ptr}(
    ...
    // Compute y-coordinate of R and (TODO) check soundness
    // %{
    //    from starkware.cairo.common.cairo_secp.secp_utils import pack
...
func recover_public_key_from_msg_hash{range_check_ptr}(
    msg_hash: BigInt3, v: felt, r: BigInt3, s: BigInt3
) -> (res: EcPoint) {
    alloc_locals;

    validate_signature_entry(r);
    validate_signature_entry(s);
    assert_in_range(v, 27, 28);

    let (res) = ecdsa_raw_recover(msg_hash, v, r, s);
    return (res=res);
}
